...
setwd("E:/Calibration/Anjeni/ScenariosforR/CalibrationwithR/Monthly/TxtInOut")
...
Qobs  <- read.table("obsdata.monthly.txt")[,4]       # obsdata needs to be copied to the folder
Qobs <- Qobs/(60*60*24*30)
Qobs.ts <- ts(Qobs, start=1986, freq=12)

Qsim1 <- read.table("output.rch", skip=9)
subsim <- subset(Qsim1, V4<13);row.names(subsim) <- NULL
Qsim <- subsim[subsim$V2==15,7]
Qsim <- Qsim[1:144]                                   # Select data from column 7 for each V2==15
#Qsim <- Qsim*2592000
...
dates <- c("1985",
           ...
           "1994","1995","1996","1997",
           "1998")                                         
yrs <- c(1985,1986,1987,1988,1989,1990,1991,1992,1993,1994,1995,1996,1997,1998)            
date.ticks<- c(0,12,24,36,48,60,72,84,96,108,120,132,144,156) 
par(mar=c(5,5,4,2))
...
box(which='plot')
legend(0, 0.13, legend=c("observed","simulated"), cex=0.8,
       lty=c(1,1), ,lwd=c(1,1),col=c("cornflowerblue", "red3"))
text(108,0.13, "Sum of squares due to regression:", pos=2, cex=0.8)
text(120,0.13, round(SSR,3), cex=0.8, pos=2)
text(108, 0.125,"Nash-Sutcliffe Efficiency:", cex=0.8, pos=2)
text(120,0.125, round(NS,3), cex=0.8,pos=2)
text(108, 0.12, "Percent bias:", cex=0.8, pos=2)
text(120, 0.12, round(PBIAS,3),cex=0.8, pos=2)
text(108, 0.115, "Root mean square error:", cex=0.8, pos=2)
text(120, 0.115, round(RMSE,3),cex=0.8, pos=2)
