...
obsFolderM <- "E:/Calibration/Anjeni/ScenariosforR/CalibrationwithR/Monthly/"

# Simulated data
simFolder <- "E:/ArcSWAT_Examples/1_Anjeni/Scenarios/AJ_WLRC_84-14_mon/TxtInOut"


# read observed data----
setwd(obsFolderM)
Qobs  <- read.table("obsdata.monthly.csv", sep=",", 
                    header=TRUE, na.strings="NA")                                # obsdata needs to be copied to the folder
Qobs <- Qobs[,5]
Qobs.ts <- ts(Qobs, start=1984, freq=12)
...
Data <- merge(Qobs, Qsim)
names(Data) <- c("Qobs", "Qsim")
...
box(which='plot')
legend(0, up+0.3, legend=c("observed","simulated"), cex=0.8,
...
Q.simu <- as.ts(Qsim, start=1986, freq=12)
Q.obs.z <- ts(Qobs, start=1986,  freq=12)
Q.simu.z <- ts(Qsim, start=1986,  freq=12)
# Plot scatterplot
