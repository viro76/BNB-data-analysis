...
setwd("C:/Modelling/Data_Paper3/6_Paper3/BasinsComparison/LowerAbbay")
...
for (i in 1:length(line)) {
  nname <- sub("[.][^.]*$", "", line[i], perl=TRUE)
  nma.X <- read.csv(line[i], header=TRUE, sep=",", na.strings="NA")
  ...
  nma.Matrix <- as.matrix(read.table(matrix[i], header=TRUE, sep=",", row.names=1, as.is=TRUE))
  ...
  for (j in seq_along(files)) {
    x <- read.csv(files[j], header=TRUE, sep=",", na.strings="-99")
    x$date <- seq(as.Date("1979-01-01"), as.Date("1979-01-01")+nrow(x)-1, by=1)
    name <- sub("[.][^.]*$", "", files[j], perl=TRUE)
    names(x) <- c(sub("[.][^.]*$", "", files[j], perl=TRUE), "date")
    x <- x[,c(2,1)]
    ...
    # 2 on one plot----
    cairo_ps(filename=paste(name, nname, "eps", sep="."), width = 11, height = 8, # "Rplot%03d.eps"
    ...
    abline(h=mean(nma.mo.mean), col="brown", lty=2)
    mtext(text=paste("NMA", substring(nname, 8), sep=" "), 3, 2.5, cex=1.5)
    mtext("1952-2003", 3, .5)
